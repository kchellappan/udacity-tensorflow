...
#network models. Remember that L2 amounts to adding a penalty on the 
#norm of the weights to the loss. In TensorFlow, you can compute the L2 
#loss for a tensor `t` using `nn.l2_loss(t)`. The right amount of 
...
with graphNN.as_default(): #Specify the default behavior of the graph as the problem statement (inputs, outputs, cost function definition, etc.)
    ...
    loss = tf.cast(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(output_layer_output, tf.cast(tf_train_labels,dtype=tf.float32))) + reg_parameter * (tf.nn.l2_loss(first_layer_weights) + tf.nn.l2_loss(second_layer_weights)),dtype=tf.float32)
...
#Let's try to reuse an existing graph, and rerun the session with fewer 
#iterations
print('')
print('Running with fewer iterations to observe overfitting')
num_iterations = 100
...
with graph_deep.as_default(): #Specify the default behavior of the graph as the problem statement (inputs, outputs, cost function definition, etc.)
    ...
    #layer, i.e, num_label softmax outputs, one for each class
    hidden_layer_output = tf.nn.relu(tf.matmul(tf_train_dataset, first_layer_weights))
    hidden_layer_output = tf.nn.dropout(hidden_layer_output,dropout_prob)
    hidden_layer_output = tf.concat(1,[tf.ones([tf_train_dataset.get_shape()[0],1],dtype=tf.float32), hidden_layer_output])
    output_layer_output = (tf.matmul(hidden_layer_output, second_layer_weights))
    
    #Define the cost function of the neural network weight optimization
    #This is the mean cross entropy error (applying softmax to output layer)    #betweem output and one-hot encoded labels
...
print('')
print('Running with 3 hidden layers, 1024 in the first, 300 in the second and 50 in the third')
dropout_prob = 1
...
with graphNN_dropout.as_default(): #Specify the default behavior of the graph as the problem statement (inputs, outputs, cost function definition, etc.)
    ...
    #Create placeholders to contain the training set (data and labels)
    tf_train_dataset = tf.placeholder(tf.float32, shape=(batch_size, image_size*image_size + 1))
    tf_train_labels = tf.placeholder(tf.float32, shape=(batch_size,num_labels))
    ...
    #cross_entropy = tf.cast(tf.reduce_mean(y * tf.log(output_layer_output + 1e-9)),dtype=tf.float32)
    ...
    learning_rate = tf.train.exponential_decay(0.04,global_step,1000,0.9,staircase=True)
    optimizer = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss, global_step=global_step)
    #Generate the predicitions made by the network on the provided data
    train_prediction = tf.nn.softmax(output_layer_output)
